...
import { getPatientColumns } from "./_components/columns";
...
export function PatientsView() {
  ...
  const columns = useMemo(() => getPatientColumns(t), [t]);

  const emptyMessage = activeQuery
    ? t("empty.search", { query: activeQuery })
    : t("empty.title");
    ...
    <Section>
      ...
      {isError ? (
        <Alert variant="destructive">
          <AlertCircle />
          <AlertTitle>{t("error.loadList")}</AlertTitle>
          <AlertDescription>
            {error instanceof Error ? error.message : t("error.retryHint")}
            <button
              type="button"
              onClick={() => refetch()}
